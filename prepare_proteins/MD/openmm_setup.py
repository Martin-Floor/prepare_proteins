try:
    ...
    raise ValueError(
        "openmm python module not avaiable. Please install it to use this function."
    )
...
class openmm_md:
    ...
    def __init__(self, input_pdb):
        ...
        self.positions = np.array(
            [c.value_in_unit(nanometer) for c in self.modeller.positions]
        )
    ...
    def setUpFF(self, ff_name):
        ...
        if ff_name not in available_ffs:
            raise ValueError(
                f"{ff_name} not found in available forcefields: {available_ffs}"
            )
    ...
    def setPeriodicBoundaryConditions(self, radius=1.5):
        ...
        vectors = [
            Vec3((max_xyz[0] - min_xyz[0]) + radius, 0, 0),
    ...
    def removeHydrogens(self, keep_ligands=True):
        hydrogens = [
            a for a in self.modeller.topology.atoms() if a.element.name == "hydrogen"
        ]
        if keep_ligands:
    ...
    def parameterizePDBLigands(
        ...
        if os.path.exists(
            parameters_folder + "/" + self.pdb_name + ".prmtop"
        ) and os.path.exists(parameters_folder + "/" + self.pdb_name + ".inpcrd"):
            if not overwrite:
                ...
                print(
                    "Parameters were already created. Give overwrite=True to recompute them."
                )
                return
        ...
        if extra_force_field:
            if extra_force_field not in extra_ffs:
                raise ValueError(
                    f"The only implemented extra ff are: {extra_ffs[0]} and {extra_ffs[1]}"
                )
            ...
            extra_force_field_dict = {}
            if extra_force_field == "parmBSC1":
                _copyFFFiles(extra_ff_folder, "parmBSC1")
                extra_force_field_dict["source"] = extra_ff_folder + "/leaprc.ff14SB"
                extra_force_field_dict["atomtypes"] = (
                    'addAtomTypes {\n { "C1" "C" "sp2" }\n { "C2" "C" "sp2" }\n { "CI" "C" "sp3" }\n { "CE" "C" "sp3" }\n }'
                )
                extra_force_field_dict["lib"] = (
                    f"loadoff {extra_ff_folder}/parmBSC1.lib"
                )
                extra_force_field_dict["frcmod"] = (
                    f"loadamberparams {extra_ff_folder}/parmBSC1.frcmod"
                )

            elif extra_force_field == "parmBSC2":
                _copyFFFiles(extra_ff_folder, "parmBSC2")
                extra_force_field_dict["source"] = extra_ff_folder + "/leaprc.bsc2"
        ...
        for r in getNonProteinResidues(
            self.modeller.topology, skip_residues=skip_ligands
        ):

            residue = r.name.upper()

            # Create PDB for each ligand molecule
            lig_top, lig_pos = topologyFromResidue(
                r, self.modeller.topology, self.modeller.positions
            )
            ...
            if residue in parameters_folders:
                for d in os.listdir(parameters_folders[residue]):
                    shutil.copyfile(
                        parameters_folders[residue] + "/" + d,
                        par_folder[residue] + "/" + d,
                    )

            if residue in parameters_mol2:
                shutil.copyfile(
                    parameters_mol2[residue],
        ...
        if extra_frcmod:
            for residue in extra_frcmod:
                if os.path.exists(residue):
                    shutil.copyfile(
                        residue, parameters_folder + "/" + residue.split("/")[-1]
                    )
                elif residue in parameters_frcmod:
                    shutil.copyfile(
                        parameters_frcmod[residue],
                    ...
                    raise ValueError(
                        f"Frcmod file for residue {residue} was not found in {metal_parameters}"
                    )
                ...
                if isinstance(extra_mol2, dict):
                    if os.path.exists(extra_mol2[residue]):
                        shutil.copyfile(
                            ...
                            parameters_folder
                            + "/"
                            + extra_mol2[residue].split("/")[-1],
                        )
                    else:
                        raise ValueError(
                            f"Mol2 file for {residue} {extra_mol2[residue]} was not found."
                        )
                else:
                    if os.path.exists(residue):
                        shutil.copyfile(
                        ...
                        raise ValueError(
                            f"Mol2 file for residue {residue} was not found in {metal_parameters}"
                        )
        ...
        for residue in par_folder:
            ...
            if residue in metal_ligand:
                ...
                for metal_residue in metal_ligand[residue]:
                    metal_pdb[metal_residue] = (
                        "../" + metal_residue + "_parameters/" + metal_residue + ".pdb"
                    )
            ...
            lig_par.getAmberParameters(
                ligand_charge=charge, overwrite=overwrite, metal_charge=metal_charge
            )
        ...
        if metal_ligand:
            ...
            if metal_parameters:
                shutil.copyfile(
                    parameters_folders["mcpbpy.frcmod"],
            ...
            for residue in metal_pdb.topology.residues():
                for r in metal_ligand:
                    if residue.name in metal_ligand[r]:
                        ...
                        if len(atoms) > 1:
                            raise ValueError(
                                "Ion residue should contain only one atom!"
                            )
            ...
            for residue in par_folder:
                ...
                if residue in metal_ligand:
                    for m in metal_ligand[residue]:
                        shutil.copyfile(
                            par_folder[residue] + "/" + m + ".mol2",
                            parameters_folder + "/" + m + ".mol2",
                        )

                shutil.copyfile(
                    par_folder[residue] + "/" + residue + ".mol2",
                    parameters_folder + "/" + residue + ".mol2",
                )
                shutil.copyfile(
                    par_folder[residue] + "/" + residue + ".frcmod",
                    parameters_folder + "/" + residue + ".frcmod",
                )
        ...
        with open(parameters_folder + "/tleap.in", "w") as tlf:
            ...
            if extra_force_field:
                tlf.write("source " + extra_force_field_dict["source"] + "\n")

                if "atomtypes" in extra_force_field_dict:
                    tlf.write(extra_force_field_dict["atomtypes"] + "\n")
                if "lib" in extra_force_field_dict:
                    tlf.write(extra_force_field_dict["lib"] + "\n")
                if "frcmod" in extra_force_field_dict:
                    tlf.write(extra_force_field_dict["frcmod"] + "\n")
            ...
            if metal_ligand:
                ...
                for residue in getNonProteinResidues(pdb.topology):
                    if residue in not_metal:
                        continue
                    missing_atoms += getMissingAtomTypes(
                        parameters_folder + "/" + residue.name + ".mol2"
                    )
    ...
    def centerPositionsAtCOM(self, output_unit=nanometer):
        masses = np.array(
            [a.element.mass.value_in_unit(amu) for a in self.modeller.topology.atoms()]
        )
...
class ligandParameters:
    ...
    def __init__(self, ligand_pdb, metal_pdb=None):
        ...
        if res_count != 1:
            raise ValueError(
                "A PDB with a single residue must be given for parameterization!"
            )

    def getAmberParameters(
        self, charge_model="bcc", ligand_charge=0, metal_charge=None, overwrite=False
    ):
        ...
        if self.metal_pdb:
            for m in self.metal_pdb:
                ...
                if not os.path.exists(metal_mol2) or overwrite:
                    ...
                    if not metal_charge:
                        raise ValueError(
                            "You must give the metal_charge if using a ligand with metal!"
                        )
