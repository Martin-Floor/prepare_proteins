...
def test_get_ligand_formal_charges_uses_source_pdb_charge_fields(tmp_path):
    pdb_text = (
    ...
    charges, report = models.getLigandFormalCharges(return_report=True)

    assert charges == {"CAR": -1}
    assert report.loc[0, "resname"] == "CAR"
    assert report.loc[0, "formal_charge"] == -1
    assert report.loc[0, "block_source"] == "source"


def test_get_ligand_formal_charges_raises_for_conflicting_reused_names(tmp_path):
    model_a = (
        _pdb_line("ATOM", 1, "CA", "ALA", "A", 1, 0.0, 0.0, 0.0, "C")
    ...
    with pytest.raises(ValueError, match="makeLigandNamesUnique"):
        models.getLigandFormalCharges()


def test_get_ligand_formal_charges_aligns_with_unique_ligand_names(tmp_path):
    model_a = (
        _pdb_line("ATOM", 1, "CA", "ALA", "A", 1, 0.0, 0.0, 0.0, "C")
        + _pdb_line("HETATM", 2, "N1", "LIG", "A", 101, 0.0, 1.0, 0.0, "N")
        + _pdb_line("HETATM", 3, "H1", "LIG", "A", 101, 0.9, 1.0, 0.0, "H")
    ...
    charges = models.getLigandFormalCharges()

    assert charges == {"L01": 0, "LIG": 1}


def test_get_ligand_formal_charges_target_names_can_force_skipped_cofactor_names(tmp_path):
    pdb_text = (
        _pdb_line("ATOM", 1, "CA", "ALA", "A", 1, 0.0, 0.0, 0.0, "C")
    ...
    assert models.getLigandFormalCharges() == {}
    assert models.getLigandFormalCharges(target_names="NAD") == {"NAD": 1}
